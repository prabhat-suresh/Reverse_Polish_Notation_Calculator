module Polynomial = struct
        ...
        let add_poly p1 p2 = let rec add_helper p1 p2 ans= match (p1,p2) with
        |(((coeff1,pow1)::xs1),((coeff2,pow2)::xs2)) -> 
                        if pow1<pow2 then if coeff1==0.0 then add_helper xs1 p2 ans else add_helper xs1 p2 ((coeff1,pow1)::ans)
                        else if pow2<pow1 then if coeff2==0.0 then add_helper p1 xs2 ans else add_helper p1 xs2 ((coeff2,pow2)::ans)
                        else if coeff1+.coeff2==0.0 then add_helper xs1 xs2 ans else add_helper xs1 xs2 ((coeff1 +. coeff2,pow1)::ans)
        |([],x2::xs2) -> add_helper [] xs2 (x2::ans)
        |((x1::xs1),[]) -> add_helper xs1 [] (x1::ans)
        | _ -> ans
                        in List.rev (add_helper p1 p2 [])
        ...
        let diff_poly = let diff_term (coeff,pow) = (coeff *. Float.of_int pow,pow-1) in List.map (fun elem -> diff_term elem)
end     
...
| "+" | "-" | "*" | "^" -> (match summ with
        |p2::p1::xs -> op p1 elem p2 :: xs 
        |_-> raise (Failure "Invalid RPN expression 2"))
|"x" -> [(1.0,1)]::summ
|"d" -> (match summ with
                        |p::xs -> Polynomial.diff_poly p :: xs
                        |_->raise (Failure "Invalid RPN expression 5"))
